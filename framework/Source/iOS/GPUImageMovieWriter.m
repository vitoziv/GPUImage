...
- (void)startRecordingAsyncInOrientation:(CGAffineTransform)orientationTransform;
{
    assetWriterVideoInput.transform = orientationTransform;
    
    [self startRecordingAsync];
}
...
- (void)finishRecordingAsync
{
    [self finishRecordingAsyncWithCompletionHandler:NULL];
}

- (void)finishRecordingAsyncWithCompletionHandler:(void (^)(void))handler
{
    recordingReadyToStop = YES;
    self.completionHandler = handler;
}

- (void)processAudioBuffer:(CMSampleBufferRef)audioBuffer;
{
    if (!allowWriteAudio || (recordingReadyToStop && (!CMTIME_IS_NEGATIVE_INFINITY(previousAudioTime))) || !isRecording) {
        return;
    }
    
    CMTime currentSampleTime = CMSampleBufferGetOutputPresentationTimeStamp(audioBuffer);
    if (recordingReadyToStop && CMTIME_IS_NEGATIVE_INFINITY(previousAudioTime)) {
        CMSampleBufferSetOutputPresentationTimeStamp(audioBuffer, previousFrameTime);
    }
...
- (void)newFrameReadyAtTime:(CMTime)frameTime atIndex:(NSInteger)textureIndex;
{
    ...
    BOOL needAudio = (!CMTIME_IS_NEGATIVE_INFINITY(previousAudioTime)) || !self.hasAudioTrack;
    if (recordingReadyToStop && CMTIME_IS_VALID(previousFrameTime) && needAudio && CMTIME_COMPARE_INLINE(previousFrameTime, >, previousAudioTime)) {
        [firstInputFramebuffer unlock];
        recordingReadyToStop =  NO;
        [self finishRecordingWithCompletionHandler:self.completionHandler];
